"""
Si（共价元素）LDA-PZ81 模守恒赝势：端到端生成 + 验证 + 导出

目的
----
1) 给出一个“可直接运行”的 Si 示例，作为 Al（金属）之外的共价对照组；
2) 验证对数导数阈值的差异化策略：Si 使用共价阈值（curve_rms_valence < 0.3）。

运行
----
python AtomPPGen/examples/validate_si_lda.py

输出
----
AtomPPGen/outputs/si_lda/
  - si_lda.json
  - si_lda.npz
"""
...
def main() -> None:
    ...
    # 一组“偏保守”的 rc 作为教学起点：共价元素通常需要更硬一些的赝势来保证相移一致性
    rc_by_l = {0: 1.8, 1: 2.0, 2: 2.2}
    ...
    report = run_full_validation(
        ...
        tm_dict=tm_dict,
        inv_dict=inv_dict,
        # 对 Si 而言，取更核外的 r_test 能减少“在测试半径恰好遇到节点”造成的 NaN 点，
        # 并让对数导数更接近核外散射区的相位信息。
        r_test=6.0,
        E_range_Ry=(-0.5, 0.5),
        E_step_Ry=0.05,
    ...
    out_dir = repo_root / "AtomPPGen" / "outputs" / "si_lda"
    out_dir.mkdir(parents=True, exist_ok=True)
    output_prefix = out_dir / "si_lda"

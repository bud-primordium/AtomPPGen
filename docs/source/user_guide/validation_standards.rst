...
- **共价元素** ：:math:`L_{\text{RMS}}^{\text{valence}} < 0.3`；节点明显、相位变化大，需更严格标准。
- 零点 RMS 主要用于确认赝势是否重现全电子的散射共振，如 ``ValidationReport`` 返回 ``Infinity``，
  表示测试窗口未包含足够零点，可通过缩小 :math:`E_{\text{step}}` 或扩大能量范围重跑。
